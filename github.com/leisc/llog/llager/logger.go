...
type LLogger interface {
	RegisterSink(Sink)
	Session(task string, data ...Data) LLogger
	SessionName() string
...
func NewLoggerExt(component string, isFormatText bool) LLogger {
	return &llogger{
		component:     component,
		task:          component,
		sinks:         []Sink{},
...
func (l *llogger) WithData(data Data) LLogger {
	return &llogger{
		component: l.component,
		task:      l.task,
		sinks:     l.sinks,
...
func (l *llogger) activeSinks(loglevel LogLevel) []Sink {
...
func (l *llogger) log(loglevel LogLevel, action string, err error, data ...Data) {
...
func (l *llogger) logs(ss []Sink, loglevel LogLevel, action string, err error, data ...Data) {
	...
	for _, sink := range l.sinks {
		...
		//logInfo = fmt.Sprint(logData)
		logInfo = fmt.Sprint(logData["log"])
...
func (l *llogger) Debug(action string, data ...Data) {
	l.log(DEBUG, action, nil, data...)
}

func (l *llogger) Info(action string, data ...Data) {
	l.log(INFO, action, nil, data...)
}

func (l *llogger) Warn(action string, data ...Data) {
	l.log(WARN, action, nil, data...)
}

func (l *llogger) Error(action string, err error, data ...Data) {
	l.log(ERROR, action, err, data...)
}

func (l *llogger) Fatal(action string, err error, data ...Data) {
	l.log(FATAL, action, err, data...)
}

func (l *llogger) logf(loglevel LogLevel, err error, format string, args ...interface{}) {
	ss := l.activeSinks(loglevel)
	if len(ss) == 0 {
		return
	}
...
func (l *llogger) Debugf(format string, args ...interface{}) {
	l.logf(DEBUG, nil, format, args...)
}

func (l *llogger) Infof(format string, args ...interface{}) {
	l.logf(INFO, nil, format, args...)
}

func (l *llogger) Warnf(format string, args ...interface{}) {
	l.logf(WARN, nil, format, args...)
}

func (l *llogger) Errorf(err error, format string, args ...interface{}) {
	l.logf(ERROR, err, format, args...)
}

func (l *llogger) Fatalf(err error, format string, args ...interface{}) {
	l.logf(FATAL, err, format, args...)
}

func (l *llogger) baseData(givenData ...Data) Data {
	data := Data{}
