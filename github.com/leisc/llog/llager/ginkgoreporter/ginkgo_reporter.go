...
func New(writer io.Writer) *GinkgoReporter {
	logger := llager.NewLogger("ginkgo")
	logger.RegisterSink(llager.NewWriterSink(writer, llager.DEBUG))
...
type GinkgoReporter struct {
	logger  llager.Logger
	writer  io.Writer
	session llager.Logger
}
...
func (g *GinkgoReporter) SpecWillRun(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		g.session.Info("start", llager.Data{
			"summary": SpecSummary{
...
func (g *GinkgoReporter) SpecDidComplete(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		if passed(specSummary.State) {
			g.session.Info("end", llager.Data{
				"summary": summary,
			})
		} else {
			summary.StackTrace = specSummary.Failure.Location.FullStackTrace
			g.session.Error("end", errorForFailure(specSummary.Failure), llager.Data{
				"summary": summary,
			})
		}
		g.session = nil
